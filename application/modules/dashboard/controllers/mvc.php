...
class Mvc extends MY_Controller {
	...
	public function index()
	{
		$data['menu'] = 'admin';
		$data['tab'] = 'admin-mvc';
		$this->_render('mvc/index',$data);
		//self::render('mvc/index');
	}

	public function create_mvc(){
        $post_data = $this->input->post();
        $tablename = $post_data['table'];
		$this->gen($tablename);
	}
	...
	private function is_auto_increment($table, $column)
	{
	    ...
	    $sql .= "TABLE_SCHEMA = '".DATABASE."' AND TABLE_NAME = '".$table."' ";
	...
	private function gen_folders($table)
	{
	...
	{
	    $filename = 'application/modules/'.$table.'/models/mdl_'.$table.'.php';
	    $fp = fopen($filename, 'w');
	    fwrite($fp, "<?php\n");
	    fwrite($fp, "\n");
	    fwrite($fp, "class Mdl_".$table." extends CI_Model {\n");
	    fwrite($fp, "\n");
	...
	{
	    $filename = 'application/modules/'.$table.'/controllers/'.$table.'.php';
	    $fp = fopen($filename, 'w');
	    fwrite($fp, "<?php\n");
	    fwrite($fp, "\n");
	    fwrite($fp, "class ".ucfirst($table)." extends MX_Controller {\n");
	    fwrite($fp, "\n");
	    fwrite($fp, "   function __construct()\n");
	    fwrite($fp, "   {\n");
	    fwrite($fp, "       parent::__construct();\n");
	    fwrite($fp, "       \$this->load->model('Mdl_".$table."');\n");
	    fwrite($fp, "   }\n");
	    fwrite($fp, "\n");
		fwrite($fp, "	private function render(\$view,\$data=NULL)\n");
		fwrite($fp, "	{\n");
		fwrite($fp, "			\$this->layout->setLayout('/layout_view');\n");
//		fwrite($fp, "			\$data['logged_in'] = $this->hmvc_auth->logged_in();\n");
		fwrite($fp, "			\$data['header_view'] = \$this->load->view('header_view',\$data,TRUE);\n");
		fwrite($fp, "			\$data['footer_view'] = \$this->load->view('footer_view',\$data,TRUE);\n");
		fwrite($fp, "			\$this->layout->view(\$view,\$data);\n");
		fwrite($fp, "	}\n");
	    ...
	    fwrite($fp, "       );\n");
	    fwrite($fp, "		\$this->render('".$table."/index',\$data);\n");
	    fwrite($fp, "   }\n");
	    fwrite($fp, "\n");
	...
	private function gen_add_controller($fp, $table)
	{
	    ...
	    fwrite($fp, "   {\n");
	    fwrite($fp, "		\$this->render('".$table."/add');\n");
	    fwrite($fp, "   }\n");
	    fwrite($fp, "\n");
	...
	private function gen_edit_controller($fp, $table)
	{
	    ...
	    fwrite($fp, "           'row' => \$row,\n");
	    fwrite($fp, "       );\n");
	    fwrite($fp, "		\$this->render('".$table."/edit',\$data);\n");
	    fwrite($fp, "   }\n");
	    fwrite($fp, "\n");
	...
	{
	    $filename = 'application/modules/'.$table.'/views/index.php';
	    $fp = fopen($filename, 'w');
	    fwrite($fp, "<link rel=\"stylesheet\" type=\"text/css\" href=\"<?php echo base_url() ?>assets/css/".$table.".css\" />\n");
	    fwrite($fp, "</head>\n");
	    fwrite($fp, "<br />\n");
	...
	{
	    $filename = 'application/modules/'.$table.'/views/add.php';
	    $fp = fopen($filename, 'w');
	    fwrite($fp, "<link rel=\"stylesheet\" type=\"text/css\" href=\"<?php echo base_url() ?>assets/css/".$table.".css\" />\n");
	    fwrite($fp, "</head>\n");
	    fwrite($fp, "<br />\n");
	    ...
	    fwrite($fp, "<script type=\"text/javascript\" src=\"<?php echo base_url() ?>assets/js/".$table.".js\"></script>\n");
	    fclose($fp);
	...
	{
	    $id = $this->id($table);
	    $filename = 'application/modules/'.$table.'/views/edit.php';
	    $fp = fopen($filename, 'w');
	    fwrite($fp, "<link rel=\"stylesheet\" type=\"text/css\" href=\"<?php echo base_url() ?>assets/css/".$table.".css\" />\n");
	    fwrite($fp, "</head>\n");
	    fwrite($fp, "<br />\n");
